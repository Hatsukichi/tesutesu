...
# 各種設定
BOARD_WIDTH  = 10   # 横コマ数
BOARD_HEIGHT = 10   # 縦コマ数
MINE_NUM     = 20   # 地雷数

MINE_BG_COLOR  = "#f000a0"     # 地雷コマ背景
FLAG_BG_COLOR  = "yellow"      # フラグコマ背景
EMPTY_BG_COLOR = "#a0a0a0"     # 何もないコマ背景
    ...
    def set_events(self):
        ...
        for j in range(self.height):
            for i in range(self.width):
                ...
                # 左クリック時のイベント設定
                label.bind("<Button-1>", self.open_cell)
                # 右クリック時のイベント設定
                if (pf == 'Windows') or (pf == 'Linux'):
                    label.bind("<Button-3>", self.raise_flag)
                else:
                    label.bind("<Button-2>", self.raise_flag)
